...
To take advantage of STM transactions in Clojure, one must use immutable and \textbf{persistent} data structures. Appropriately enough, this is the case for \textit{all} data structures in Clojure. The use of persisitent and immutable data structures combined with separation of values and identity creates a high performing concurrency environment\footnote{Highest performance is achieved in environments with high-read and low-write that implies low collision frequency.} without the need of lock-based programming.
...
The idea of persistent collections combined with transactions greatly enhance performance. Since each transaction when invoked fetch a copy of the state, copying an persistent collection is quickly executed but implies storage-overhead arguably a fair compromise if the increased concurrency.
...
\part{Conclusion}
Write-skew anomaly, compiler affecting execution, ...
